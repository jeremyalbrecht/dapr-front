<template>
  <div>
    <h2 class="text-2xl font-bold mb-4 text-center">
      Manage Orders
      <span v-if="orders_ms_port === 18081" class="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">Azure</span>
      <span v-else-if="orders_ms_port === 28081" class="bg-orange-100 text-orange-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">AWS</span>
    </h2>
      ...
      <div
        ...
        <div class="mt-6 flex space-x-4">
          ...
          <button
              ...
              'bg-blue-500 text-white hover:bg-blue-600': payment_ms_port === 15000,
              'bg-orange-500 hover:bg-orange-600': payment_ms_port === 25000,
            }">
...
export default {
  ...
  data() {
    return {
      ...
      payment_ms_port: this.payment_ms_port
    }
  },
  setup() {
    const orders = ref([]);
    ...
    const articlesMsPort = proxy.articles_ms_port;

    const fetchOrders = async () => {
      const response = await axios.get('http://localhost:' + articlesMsPort + '/api/order/get');
  ...
  methods: {
    async payOrder(id) {
      try {
        const response = await axios.post('http://localhost:' + this.orders_ms_port + '/payOrder', {id: id});
